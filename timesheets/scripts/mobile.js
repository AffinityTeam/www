...
Affinity.MobileEnabled = false;
Affinity.EnableMobile = function ()
{
  if (Affinity.MobileEnabled) return;
  Affinity.MobileEnabled = true;
  ...
  if (MooTools && 'version' in MooTools && parseFloat(MooTools.version) >= 1.5 && parseFloat(MooTools.version) < 1.6) {
  ...
  Affinity.stopHammerEvent = function (e)
  {
    ...
    if (event)
    {
      ...
      try
      {
        if (e.hasOwnProperty('event') && e.event.hasOwnProperty('cancelable') && e.event.cancelable) e.preventDefault();
      } catch (_ex) { }
    }
    if (srcEvent)
    {
