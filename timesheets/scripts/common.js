...
Affinity.Components.Commons.Version = '6.0.34.2';
...
Affinity.debug = !(parseInt(Affinity.Components.Commons.Version.split('.')[2]) % 2 === 0);
...
Affinity.mobile = false;
Affinity.istablet = false;
Affinity.tablet = false;
 ...
 * @returns {boolean}
 */
if (!Affinity.hasOwnProperty('isString'))
{
  Affinity.isString = function (obj)
  {
    return typeof obj === 'string';
  };
  Affinity.IsString = Affinity.isString;
}
 ...
 * @param {Event} ev
 */
if (!(Affinity.hasOwnProperty('stopEvent')))
{
  Affinity.stopEvent = function (ev)
  {
    if (ev instanceof Event)
    {
      ...
      if ('srcEvent' in ev)
      {
        try { ev.srcEvent.stopPropagation(); } catch (er) { console.warn('failed to stop src event propagation'); };
      }
      else
      {
  ...
  Affinity.StopEvent = Affinity.stopEvent;
}


/**
* Affinity.openLink(String, String)
* Open url in new browser window/tab
*
* @author  Ben King, ben.king at source63.com, +64 21 2672729.
*
* @param {string} link Url to open
* @param {string} target Window / Tab to open into (default _blank)
*/
if (!(Affinity.hasOwnProperty('openLink')))
{
  Affinity.openLink = function (link, target)
  {
    target = Affinity.IsString(target) ? target : '_blank';
    if (!document.querySelector('#linker'))
    {
      Affinity.linker = document.createElement('a');
      Affinity.linker.setAttribute('target', target);
      Affinity.linker.className = 'obscured';
      document.body.appendChild(Affinity.linker);
    }
    Affinity.linker.setAttribute('href', link);
    Affinity.linker.click();
    Affinity.linker.setAttribute('href', null);
  };
  Affinity.OpenLink = Affinity.openLink;
}
  ...
  DashboardWrapper: function ()
  {
    ...
    for (; i < checkKeys.length; i++)
    {
      ...
      if (Affinity.hasOwnProperty(key))
      {
        ...
        {
          if (typeOf(Affinity[key]) === 'string' && Affinity[key].trim() !== '')
          {
            hasDashboard = Affinity[key].trim().toLowerCase() === 'true';
          }
        }
      }
    }
...
Affinity.PageReady = function (ev)
{
  ...
  Affinity.IeWarning = function ()
  {
    ...
    //if (Affinity.debug)
    //{
    //  Affinity.Browser.isie = true;
    //  Affinity.Browser.isedge = false;
    //}
    if (typeof window.masterMessage !== 'undefined' && Affinity.ShowIeWarning && Affinity.Browser.isie && !Affinity.Browser.isedge)
    {
      var message = '';
...
/**   DEVICE CHECK               *****************************************************************************************/
/**   ------------------------   *****************************************************************************************/
/**   Cehck device type and values.                                                                                     **/

Affinity.MobileDetect = {
  ...
  Start: function ()
  {
    ...
    if (document.querySelector('body')) Affinity.MobileDetect.getBrowser();
    else setTimeout(Affinity.MobileDetect.checkBody, 10);
  },
  ...
  getBrowser: function ()
  {
    ...
    let loaded = function ()
    {
      ...
      Affinity.Browser.platform = result.os.name + (result.os.version !== undefined ? ' ' + result.os.version : '');
      Affinity.Browser.renderer = result.engine.name;
      Affinity.Browser.name = result.browser.name;
      Affinity.Browser.version = result.browser.version;
      Affinity.Browser.major = parseInt(result.browser.major);
      Affinity.Browser.minor = result.browser.version.replace(result.browser.major, '');
      Affinity.Browser.minor = Affinity.Browser.minor.indexOf('.') === 0 ? Affinity.Browser.minor.substr(1) : Affinity.Browser.minor;
      Affinity.Browser.minor = Affinity.Browser.minor.indexOf('0.') === 0 ? Affinity.Browser.minor.substr(2) : Affinity.Browser.minor;
      Affinity.Browser.minor = parseFloat(Affinity.Browser.minor);
      if (Affinity.Browser.name.toLowerCase().contains('ie')) Affinity.Browser.isie = true;
      if (Affinity.Browser.name.toLowerCase().contains('edge')) Affinity.Browser.isedge = true;
      if (Affinity.Browser.name.toLowerCase().contains('chrome')) Affinity.Browser.ischrome = true;
      if (Affinity.Browser.name.toLowerCase().contains('safari')) Affinity.Browser.issafari = true;
      if (Affinity.Browser.name.toLowerCase().contains('firefox')) Affinity.Browser.isfirefox = true;
      if (Affinity.Browser.platform.toLowerCase().contains('android')) Affinity.Browser.isandroid = true;
      if (Affinity.Browser.platform.toLowerCase().contains('mac')) Affinity.Browser.ismac = true;
      if (Affinity.Browser.platform.toLowerCase().contains('ios')) Affinity.Browser.ismac = true;
      if (result.device.model !== undefined && result.device.model.toLowerCase().contains('ipad')) Affinity.Browser.isipad = true;
      if (Affinity.Browser.isipad && !Affinity.Browser.ismac) Affinity.Browser.ismac = true;
      Affinity.MobileDetect.getWurfl();
    };
  ...
  getWurfl: function ()
  {
    ...
    {
      Affinity.Browser.widemode = window.innerWidth >= 1024;
      if (!Affinity.Browser.ismobile)
      {
        if (('ontouchstart' in window) || (navigator.maxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0))
        {
          Affinity.Browser.ismobile = true;
        }
      }
      var tablet = false;
      if (navigator.platform === 'MacIntel') Affinity.Browser.ismac = true;
      if (!Affinity.Browser.ismobile && (Affinity.Browser.platform.toLowerCase().contains('ios') || Affinity.Browser.platform.toLowerCase().contains('android'))) Affinity.Browser.ismobile = true;
      if (Affinity.Browser.ismobile && Affinity.Browser.widemode) Affinity.Browser.istablet = true;
      if (Affinity.Browser.istablet && Affinity.Browser.ismac) Affinity.Browser.isipad = true;
      if (Affinity.Browser.istablet && Affinity.Browser.widemode && !Affinity.Browser.isipad) Affinity.Browser.ismobile = false;
      var body = document.body;
      body.classList.remove('mobile', 'tablet', 'desktop', 'edge', 'ie', 'firefox', 'mac', 'wide-mode');

      if (!Affinity.Browser.ismobile || (Affinity.Browser.istablet && Affinity.Browser.widemode && !Affinity.Browser.isipad)) body.classList.add('desktop');
      if (!body.classList.contains('desktop'))
      {
        if (Affinity.Browser.ismobile) body.classList.add('mobile');
        if (Affinity.Browser.istablet) body.classList.add('tablet');
        if (Affinity.Browser.istablet && Affinity.Browser.widemode) body.classList.add('wide-mode');
      }

      if (Affinity.Browser.ismac) body.classList.add('mac');
      if (Affinity.Browser.isipad) body.classList.add('ipad');
      if (Affinity.Browser.isie && !isNaN(parseInt(Affinity.Browser.major))) body.classList.add('ie' + parseInt(Affinity.Browser.major));
      if (Affinity.Browser.isedge) body.classList.add('edge');
      if (Affinity.Browser.isfirefox) body.classList.add('firefox');
      Affinity.mobile = Affinity.ismobile = Affinity.IsMobile = Affinity.isMobile = Affinity.Browser.ismobile;
      if (Affinity.mobile && Affinity.hasOwnProperty('EnableMobile')) Affinity.EnableMobile();
      window.dispatchEvent(new Event('MobileChecked'));
      Affinity.PageReady();
    };
    script.onload = function ()
    {
      Affinity.MobileChecked = true;
      Affinity.Browser.ismobile = WURFL.is_mobile;
      Affinity.Browser.device = WURFL.form_factor.toLowerCase();
      scriptProcessed();
    }
    script.onerror = function ()
    {
      ...
      Affinity.MobileChecked = true;
      Affinity.Browser.ismobile = false;
      Affinity.Browser.device = 'unknown';
      scriptProcessed();
    };
