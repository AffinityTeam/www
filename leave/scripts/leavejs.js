...
var UILeaveApply = new Class({
    ...
    updateUnits: function (units, unitType) {
        ...
        for (var i = 0; i < units.length; i++) {
            var unit = units[i];
            var dateInput = Date.parse(this.leavePeriodDaysContainer.getElementsByClassName('leave-apply-input-date')[i].value).toLocaleString();

            if (dateInput == Date.parse(unit.Date).toLocaleString()) {
                var hoursInput = parseInt(this.leavePeriodDaysContainer.getElementsByClassName('leave-apply-input-hours')[i].value);
                var daysInput = parseInt(this.leavePeriodDaysContainer.getElementsByClassName('leave-apply-input-days')[i].value);
...
var UILeaveDetail = new Class({
    ...
    viewDetail: function () {
       ...
       // this.leaveProjectionPlaceHolder = new Element('div', { 'class': 'leave-detail-leave-projection-placeholder' }).inject(this.leaveDetailsGroupRow2);
        ...
        //Row6 Columns
        this.leaveDetailsRow6Column1 = new Element('span', { 'class': 'leave-detail-group-column-blank-container' }).inject(this.leaveDetailsGroupRow6);
        ...
        if (this.isManager) {
            ...
            this.managerCommentEdit = new InputEditWidget({
                ...
                updateInput: function () {
                    ...
                    if (newManagerComment != this.data.LeaveHeader.Reply) {
                        var vm = this;
                        var customErrorHandler = function (responseObject,vm) {
                            var errMessage = "Something's stopping the Manager Comments field from updating. Try again.<br /><br />";
    ...
    leaveReasonSelectorChanged: function (vm) {
        ...
        if (vm.isInEditMode) {
            ...
            vm.updateLeave2(null, null, customErrorHandler, vm);
        ...
        if (leave.StatusCode !== 6) {
            if (this.isManager && !leave.isExternal) {
                ...
                this.declineButton.addEvent('click', function () {
                    ...
                    this.updateLeave2(2, customResponse, null, vm);
                }.bind(this));
            }
                ...
                if (leave.StatusCode !== -1) {
                    ...
                    this.approveButton.addEvent('click', function () {
                        if (this.validateAttachmentRequirement()) {
                            ...
                            this.updateLeave2(3, customResponse, null, vm);
                        } else {
                            this.displayAttachmentRequiredModalMessage();
                        }
    ...
    setUnitValue: function (dateCriteria, newDaysValue, newHoursValue) {
    ...
    getEditedLeaveInstance: function (excludeDaysWithNoScheduledHours) {
        ...
        editedLeaveInstance.timeLastModified = currentLeaveObject.TimeLastModified; //this.timeLastModifiedField.get('value');
        if (this.isManager) {
        ...
        var leaveUnitHours = this.leavePeriodDaysContainer.getElementsByClassName('leave-detail-input-hours');
        var leaveUnitDays = this.leavePeriodDaysContainer.getElementsByClassName('leave-detail-input-days');//leave-detail-input-small
        var leaveUnitScheduledHours = this.leavePeriodDaysContainer.getElementsByClassName('leave-detail-input-small');
